				...
				// 				http://docs.ckeditor.com/#!/guide/plugin_sdk_sample_1
				{
					...
					children : [
					{
						...
						onKeyUp: function() {
							...
							var number = element_id.split('_');
							var the_id = parseInt(number[1]) + parseInt(3);
							console.log(the_id)
							var select = $('#cke_'+the_id+'_select').get(0);
							
							
							for (var i = 0; i < select.length; i++) {
							        var txt = select.options[i].text;
							        var include = txt.toLowerCase().indexOf(title.toLowerCase());
